...
*Generated by Claude via the Pinch MCP tools — no custom prompting, just "tell me something interesting."*
...
> Connect to the Pinch MCP server at `http://localhost:4000/api/mcp`
...
On first interaction, the AI will call `get_started` to learn about Pinch's conventions, then walk you through the full onboarding flow: setting your timezone, entering your current cash balance, adding savings accounts, and setting up investment tracking with automatic market prices.
...
Pinch is designed for self-hosted, single-user use. The recommended setup is to run it behind [Tailscale](https://tailscale.com/) — no auth layer needed, Tailscale provides mutual WireGuard authentication at the network level. Works on desktop, iOS, and Android.
