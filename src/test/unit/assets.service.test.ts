...
describe("AssetService", () => {
  ...
  it("EUR deposit with no price uses €1 fallback for currentValue", () => {
    ...
    // Buy 1000 units (€1000)
    lotService.buy(asset.id, { quantity: 1000, pricePerUnit: 1, date: "2026-01-01" });
  ...
  it("computes correct metrics with price snapshot", () => {
    ...
    lotService.buy(asset.id, { quantity: 10, pricePerUnit: 345.63, date: "2026-01-01" });
...
describe("AssetLotService", () => {
  ...
  it("sell auto-generates 'Withdraw' description for deposit assets", () => {
    const asset = assetService.create({ name: "Savings", type: "deposit", currency: "EUR" });
    lotService.buy(asset.id, { quantity: 5000, pricePerUnit: 1, date: "2026-01-01" });
  ...
  it("sell creates a transfer transaction and negative lot", () => {
    const asset = assetService.create({ name: "BTC", type: "crypto", currency: "EUR" });
    lotService.buy(asset.id, { quantity: 1, pricePerUnit: 80000, date: "2026-01-01" });
  ...
  it("sell throws when quantity exceeds holdings", () => {
    const asset = assetService.create({ name: "ETF", type: "investment", currency: "EUR" });
    lotService.buy(asset.id, { quantity: 5, pricePerUnit: 100, date: "2026-01-01" });
  ...
  it("buy throws for EUR deposit with pricePerUnit !== 1", () => {
    const asset = assetService.create({ name: "Savings", type: "deposit", currency: "EUR" });
    expect(() =>
      lotService.buy(asset.id, { quantity: 1, pricePerUnit: 5000, date: "2026-01-01" })
    ).toThrow("pricePerUnit must be 1");
  });

  it("buy accepts EUR deposit with pricePerUnit === 1", () => {
    const asset = assetService.create({ name: "Savings", type: "deposit", currency: "EUR" });
  ...
  it("currentHoldings is free of IEEE 754 float noise", () => {
    const asset = assetService.create({ name: "BTC", type: "crypto", currency: "EUR" });
    lotService.buy(asset.id, { quantity: 0.003, pricePerUnit: 80000, date: "2026-01-01" });
    lotService.buy(asset.id, { quantity: 0.003, pricePerUnit: 81000, date: "2026-02-01" });
    lotService.buy(asset.id, { quantity: 0.003, pricePerUnit: 82000, date: "2026-03-01" });
  ...
  it("costBasis (FIFO): partial sell consumes oldest lot first", () => {
    ...
    // Buy 5 at €102.50
    lotService.buy(asset.id, { quantity: 5, pricePerUnit: 102.5, date: "2026-03-01" });
    // Sell 2 — FIFO consumes 2 from the single lot, remaining 3 @ €102.50
    lotService.sell(asset.id, { quantity: 2, pricePerUnit: 112, date: "2026-03-15" });
  ...
  it("costBasis (FIFO): sell consumes the first buy lot, leaving second", () => {
    const asset = assetService.create({ name: "BTC", type: "crypto", currency: "EUR" });
    lotService.buy(asset.id, { quantity: 1, pricePerUnit: 80000, date: "2026-01-01" });
    lotService.buy(asset.id, { quantity: 1, pricePerUnit: 90000, date: "2026-02-01" });
    // Sell 1 — FIFO consumes the first lot (€80k), leaving 1 @ €90k
    lotService.sell(asset.id, { quantity: 1, pricePerUnit: 95000, date: "2026-03-01" });
  ...
  it("costBasis (FIFO): fully closed position then re-buy resets cost basis", () => {
    ...
    lotService.buy(asset.id, { quantity: 15, pricePerUnit: 300, date: "2026-01-01" });
    lotService.sell(asset.id, { quantity: 15, pricePerUnit: 320, date: "2026-01-15" });
    lotService.buy(asset.id, { quantity: 1, pricePerUnit: 20, date: "2026-02-01" });
  ...
  it("listLots returns lots ordered by date descending", () => {
    const asset = assetService.create({ name: "ETF", type: "investment", currency: "EUR" });
    lotService.buy(asset.id, { quantity: 1, pricePerUnit: 100, date: "2026-01-01" });
    lotService.buy(asset.id, { quantity: 2, pricePerUnit: 105, date: "2026-03-01" });
...
describe("PortfolioService", () => {
  ...
  it("cash balance is income minus expenses plus signed transfers", () => {
    txService.create({ amount: 1000, type: "income", description: "Salary", date: "2026-03-01" });
    txService.create({ amount: 300, type: "expense", description: "Rent", date: "2026-03-01" });
  ...
  it("net worth includes asset values and buying is net-worth-neutral", () => {
    ...
    lotService.buy(asset.id, { quantity: 10, pricePerUnit: 345.63, date: "2026-03-01" });
  ...
  it("selling an asset is net-worth-neutral at the same price", () => {
    ...
    lotService.sell(asset.id, { quantity: 4, pricePerUnit: 345.63, date: "2026-03-20" });
  ...
  it("allocation percentages sum to 100 for a single asset", () => {
    const asset = assetService.create({ name: "BTC", type: "crypto", currency: "EUR" });
    lotService.buy(asset.id, { quantity: 1, pricePerUnit: 80000, date: "2026-03-01" });
